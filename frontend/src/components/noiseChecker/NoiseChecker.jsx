...
const NoiseChecker = () => {
  ...
  const startNoiseCheck = async () => {
    try {
      ...
      dispatch({
        type: "SET_NOISE_LEVEL_UPDATE_INTERVAL",
        payload: noiseCheckInterval,
      });
    } catch (error) {
  ...
  return (
    <>
      ...
      <Text>
        We will conduct an Environmental Noise Check before starting the test.
      </Text>
      ...
      {routeName === "Parental Control Noise Check" ? (
        state.isNoiseChecking === false ? (
          <ButtonFunc
            handleOnPress={startNoiseCheck}
            disabled={
              !state.isMicrophonePermGranted || !state.isRecordingPermGranted
            }
            text={"START NOISE CHECK"}
          />
        ) : (
        ...
        <ButtonFunc
          handleOnPress={() => {
            state.isNoiseChecking
              ? (stopNoiseCheck(), navigation.navigate("Tutorial"))
              : startNoiseCheck();
          }}
          isDisabled={
            !state.isMicrophonePermGranted ||
          ...
          text={
            state.isNoiseChecking && state.safeDuration > 3
              ? "Proceed to Test"
              : "CHECK"
          }
        />
        ...
        <>
          {state.noiseLevel <= -12 && (
            <Card backgroundColor={Colors.secondary.g5} margin={16}>
              ...
              <Text>
                No risk of hearing loss, no matter how long you listen.
              </Text>
            </Card>
          )}
